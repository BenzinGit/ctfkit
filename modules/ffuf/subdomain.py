...
def show_menu():
    ...
    print(
        f"{B}┌── {BOLD}SUBDOMAIN FUZZING{W}{B} ─────────────────────┐{W}"
    )
...
def run(data, cred, args):
    ...
    outfile = (
        artifact_dir
        /
        f"subdomain_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    print(
        f"{B}┌── {BOLD}MODULE: SUBDOMAIN FUZZING{W}{B} ─────────────┐{W}"
    )
    ...
    print(
        f"{B}│{W} Threads: {C}{config['threads']:<38}{W}{B}│{W}"
    )

    print(
        f"{B}│{W} Output:  {C}{outfile.name:<38}{W}{B}│{W}"
    )
