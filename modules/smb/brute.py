...
def run(data, cred, args):
    ...
    if not hasattr(args, "extra") or not args.extra:
        ...
        print(f"  {Y}ctf smb.brute {M}<name>{W}")
        print(f"  {Y}ctf smb.brute {M}<name> <password.list>{W}")
        print(f"  {Y}ctf smb.brute {M}<usernames.list> <passwords.list>{W}\n")
    ...
    if getattr(args, "ref", False):
        ...
        print(
            f"\n{B}┌── {BOLD}MODULE: SMB BRUTE FORCE{W}{B} ─────────────────────┐{W}"
        )
